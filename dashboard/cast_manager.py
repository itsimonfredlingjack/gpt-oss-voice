...
def _healthcheck_url(dashboard_url: str) -> str:
    ...
    Prefers DASHBOARD_HEALTHCHECK_URL if set; otherwise uses /api/status on the
    same scheme+host+port as dashboard_url.
    """
    ...
    return urlunsplit((parts.scheme, parts.netloc, "/api/status", "", ""))
